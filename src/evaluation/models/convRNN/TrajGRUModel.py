...
class TrajGRU(BaseConvRNN):
    ...
    def __init__(self, input_channel, num_filter, b_h_w, device, zoneout=0.0, L=5,
    ...
    def forward(self, inputs=None, states=None, seq_len=3):
        ...
        if inputs is not None:
            seq_len, B, C, H, W = inputs.size()
            i2h = self.i2h(torch.reshape(inputs, (-1, C, H, W)))
            i2h = torch.reshape(i2h, (seq_len, B, i2h.size(1), i2h.size(2), i2h.size(3)))
        ...
        for i in range(seq_len):
            ...
            for j in range(len(flows)):
                flow = flows[j]
                wrapped_data.append(wrap(prev_h, -flow, self.device))
