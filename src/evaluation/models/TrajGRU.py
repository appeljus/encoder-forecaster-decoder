...
from evaluation.models.convRNN.net_params import get_traj_gru_encoder_params, get_traj_gru_forecaster_params
from models.EncoderDecoder.PassThrough import PassThrough
from models.Recurrent.Recurrent import Recurrent


class TrajGRU(Recurrent):
    def __init__(self,
                 device,
                 do_persistence,
                 ...
                 forecast_horizon=1,
                 model_params=None):
        super().__init__(device, debug, do_persistence)

        self.backprop_all_outputs = backprop_all_outputs
        self.debug = debug
        self.do_persistence = do_persistence

        encoder_params = get_traj_gru_encoder_params(batch_size, device)
        forecaster_params = get_traj_gru_forecaster_params(batch_size, device)
